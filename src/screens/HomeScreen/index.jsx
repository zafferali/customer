...
import { FlatList, ScrollView, StyleSheet, Text, View, ActivityIndicator, Image, TouchableOpacity, Modal, TextInput, SafeAreaView } from 'react-native'
import firestore from '@react-native-firebase/firestore'
import { useDispatch, useSelector } from 'react-redux'
import { setRestaurants, setCurrentRestaurant } from 'slices/restaurantsSlice'
  ...
  useEffect(() => {
    ...
    const fetchRestaurants = async () => {
      try {
        ...
        let fetchedRestaurants = await Promise.all(snapshot.docs.map(async (doc) => {
          const data = doc.data()
          const menuSnapshot = await firestore().collection('restaurants').doc(doc.id).collection('menu').get()
        ...
        fetchedRestaurants.forEach(restaurant => {
          ...
          if (manualOverride && moment(manualOverride.date).isSame(todayDate, 'day')) {
            ...
            const todayOccasion = occasions ? occasions.find(occasion => moment(occasion.date).isSame(todayDate, 'day')) : null

            if (todayOccasion) {
              isOpenNow = todayOccasion.isOpen && isTimeSlotWithin(selectedTimeSlot, todayOccasion.from, todayOccasion.until)
              if (!isOpenNow) {
            ...
            if (!isOpenNow && !todayOccasion) {
              const currentDay = restaurant?.availability.general ? restaurant?.availability.general[dayOfWeek] : null
  ...
  useEffect(() => {
    const filterRestaurants = (restaurants, query) => {
      ...
      return restaurants.filter(restaurant =>
        ...
        (restaurant?.cuisines && restaurant?.cuisines.some(cuisine => cuisine.toLowerCase().includes(query.toLowerCase())))
      )
    }
        ...
        return restaurants.filter(restaurant => {
          const hasMatch = restaurant.menuItems.some(item =>
            categoryWords.some(categoryWord =>
              item.toLowerCase().includes(categoryWord)
            )
          )
          if (hasMatch) {
    ...
    filteredUnavailable.length > 0 && (
      <>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Restaurant onPress={() => handleRestaurantPress(item)} data={item} style={[styles.card, styles.unavailable]} availabilityText={item.nextAvailableFrom} />
          )}
  ...
  return (
    <Layout title='Home' navigation={navigation}>
      <TouchableOpacity onPress={() => navigation.navigate('TimeSlotScreen')} style={styles.topBar}>
        ...
        <Image style={styles.chevron} source={require('images/right.png')} />
      </TouchableOpacity>
      <SearchBar
      ...
      <View style={styles.categoriesContainer}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {categories.map(category => (
            <TouchableOpacity key={category.id} style={styles.categoryItem} onPress={() => handleCategoryPress(category.name)}>
